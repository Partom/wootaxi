...
<div class="col-md-12">
    <form class="form-horizontal" role="form" method="POST" action="{{ url('/provider/register') }}">
        ...
        <input id="service_color" type="text" class="form-control" name="service_color" value="{{ old('service_color') }}" placeholder="Car Color">

        @if ($errors->has('service_color'))
            <span class="help-block">
                <strong>{{ $errors->first('service_color') }}</strong>
            </span>
        @endif
